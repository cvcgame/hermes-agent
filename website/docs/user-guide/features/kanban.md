...
- A **Kanban** tab showing one column per status: `triage`, `todo`, `ready`, `running`, `blocked`, `done` (plus `archived` when the toggle is on).
  - `triage` is the parking column for rough ideas and human decisions. With the default `kanban.auto_decompose: true`, the dispatcher consumes only **explicit current-generation decomposition intent** created by a deliberate Decompose approval; it **does not fan out ordinary triage tasks or ordinary approval choices**. The built-in decomposer uses the `auxiliary.kanban_decomposer` model path, reads your profile roster (with descriptions), and fans an approved task out into a small graph of child tasks routed to the best-fit specialists. The original task stays alive as the parent of every child so its assignee (`kanban.orchestrator_profile`, or the active default profile when unset) wakes back up to judge completion when everything finishes. Direct `hermes kanban decompose <id>` / `--all` and dashboard **⚗ Decompose** actions remain deliberate manual operations in either mode; `hermes kanban specify` remains available for a single-task spec rewrite.
...
The kanban board has automatic consumption of approved intent and deliberate manual commands for Triage tasks:

**Auto (default)** — `kanban.auto_decompose: true`. A deliberate approval choice with the `decompose` action records **explicit current-generation decomposition intent**. The gateway-embedded dispatcher consumes only tasks carrying that current intent, capped by `kanban.auto_decompose_per_tick` (default 3 intents per tick). It **does not fan out ordinary triage tasks or ordinary approval choices**. Before writing anything, the decomposition path revalidates that the same packet generation and choice are still current; a later block, unblock, approval resolution, or decomposition invalidates stale intent. The decomposer uses the built-in prompt plus `auxiliary.kanban_decomposer`, reads installed profiles and descriptions, and produces either a routed child-task graph or a specify-style single-task result. After fan-out, the original task waits on the children and returns to `ready` for its assignee (`kanban.orchestrator_profile`, or the active default profile when unset) to judge completion.

**Manual** — `kanban.auto_decompose: false`. The dispatcher does not consume approval-created decomposition intent. Direct commands remain available: click **⚗ Decompose** on a card, run `hermes kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. `hermes kanban specify <id>` / `--all` likewise remains the explicit single-task rewrite path. These manual CLI/dashboard actions run when invoked and are distinct from generic triage status.

**Important boundary:** `auto_decompose` gates only the dispatcher's consumption of explicit intent. It does not prevent direct Decompose/Specify commands, a profile from calling `kanban_create`, or creator-session wake-ups. With `kanban.auto_subscribe_on_create: true`, a task's terminal event resumes the originating agent with a synthetic status turn so it can inspect the handoff and decide whether genuinely new follow-up work is needed. Set `auto_subscribe_on_create: false` when task completion should remain passive. For provenance, built-in decomposer children use `created_by=auto-decomposer`; tasks created by a resumed profile carry that profile name instead.
...
| Key | Default | Purpose |
|---|---|---|
| `auto_decompose` | `true` | Lets the dispatcher consume explicit current-generation decomposition intent created by a deliberate Decompose approval. It does not fan out ordinary triage tasks or ordinary approval choices, and it does not gate direct Decompose/Specify commands, profile-driven `kanban_create` calls, or creator wake turns. |
| `auto_decompose_per_tick` | `3` | Cap on current explicit intents consumed per dispatcher tick. Excess intent waits for the next tick. |
