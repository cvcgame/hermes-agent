...
- **Triage** — raw ideas and tasks awaiting a human decision. With the default `kanban.auto_decompose: true`, the dispatcher consumes only **explicit current-generation decomposition intent** created by a deliberate Decompose approval; it **does not fan out ordinary triage tasks or ordinary approval choices**. A current intent invokes `auxiliary.kanban_decomposer`, which reads your profile roster and descriptions and produces a routed child-task graph (or a single-task fallback). Flip the **Orchestration: Auto/Manual** pill to control that automatic intent consumption. Direct **⚗ Decompose**, `hermes kanban decompose <id>` / `--all`, and `/kanban decompose <id>` actions remain deliberate manual operations in either mode. **✨ Specify** and `hermes kanban specify <id>` / `--all` remain the explicit one-shot spec rewrite. See [Auto vs Manual orchestration](./kanban#auto-vs-manual-orchestration) in the main Kanban guide.
