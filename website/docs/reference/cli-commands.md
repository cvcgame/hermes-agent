...
| `decompose <id>` / `decompose --all` | Deliberately fan a triage-column task out into a graph of child tasks routed to specialist profiles by description. Falls back to specify-style single-task promotion when the LLM decides the task does not benefit from fan-out. Same flags as `specify`; direct CLI/dashboard commands work whether `kanban.auto_decompose` is true or false. Configure the model under `auxiliary.kanban_decomposer`; `kanban.orchestrator_profile` controls only the root owner after fan-out. With `auto_decompose: true`, the dispatcher consumes explicit current-generation decomposition intent created by a deliberate Decompose approval; it does not fan out ordinary triage tasks or ordinary approval choices. See [Auto vs Manual orchestration](/user-guide/features/kanban#auto-vs-manual-orchestration). |
